...
void epd_lcd_set_pixel_clock_MHz(int frequency) {
    ...
    uint32_t freq = lcd_hal_cal_pclk_freq(&lcd.hal, 240000000, lcd.config.pixel_clock, flags, &clk_div);
    lcd_ll_set_group_clock_coeff(&LCD_CAM, (int)clk_div.integer, (int)clk_div.denominator, (int)clk_div.numerator);
